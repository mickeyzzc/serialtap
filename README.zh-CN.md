...
- [架构](docs/zh-CN/architecture.md) —— 包分层与依赖规则、采集器状态机、open-once-and-hold、
  release/flash 编排、可测性 seam 一览
- [配置参考](docs/zh-CN/configuration.md) —— 全部字段与默认值、设备命名链、PAUSED 文件、
  内置签名表、日志轮转
- [控制协议](docs/zh-CN/control-protocol.md) —— ctl socket 的 JSON 行协议完整语义（脚本化集成）
