...
- 同一守护进程内撞名自动加**身份派生后缀**：`-<4位散列>`，散列输入是设备
  稳定身份（key + by-id，Windows 实例路径内嵌 MAC）——同一块板无论第几个
  接入、跨守护重启后缀一致（对比接入顺序计数 `-2`，会随插拔顺序漂移）；
  散列也撞时退回 `-2`/`-3` 计数保底
- 双板同芯片并存时：裸基名先到先得（重启后归属可能换主），**请锚定后缀名**
  （如 `^esp32s3-jtag-1x2y$`）或用配置 `names` 按 by-id 序列号/MAC 起语义名
